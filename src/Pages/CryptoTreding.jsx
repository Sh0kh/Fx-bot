...
const INTERVAL = "15m";
const LIMIT = 500;
const MIN_SIGNAL_DURATION = 30 * 60 * 1000; // 30 минут в миллисекундах
const HYSTERESIS_THRESHOLD = 2; // Разница в баллах для смены сигнала
...
// Конвертация символа для Binance
const formatSymbolForBinance = (symbol) => symbol.replace("/", "").concat("T");
...
export const fetchHistoricalData = async (symbol, setCurrentPrices) => {
    try {
        ...
        setCurrentPrices(prev => ({ ...prev, [symbol]: closes[closes.length - 1] }));
...
const calculateIndicators = (data) => {
    ...
    try {
        ...
        const macd = MACD.calculate({ fastPeriod: 16, slowPeriod: 32, signalPeriod: 9, values: closes });
...
// Анализ входа с гистерезисом
const analyzeEntry = (indicators) => {
    if (!indicators || indicators.isLowVolume) {
        return { signal: "HOLD", conditionsMet: { bullish: 0, bearish: 0 } };
    }

    const { ema20, sma50, sma200, emaRsi, macd, bollingerBands, priceAction, lastPrice, hasPinBar } = indicators;
    ...
    // Гистерезис - для смены сигнала нужно больше подтверждений
    if (bullishScore > bearishScore + HYSTERESIS_THRESHOLD) {
        return { signal: "BUY", conditionsMet: { bullish: bullishScore, bearish: bearishScore, total: 6 } };
    }

    if (bearishScore > bullishScore + HYSTERESIS_THRESHOLD) {
        return { signal: "SELL", conditionsMet: { bullish: bullishScore, bearish: bearishScore, total: 6 } };
    }

    return { signal: "HOLD", conditionsMet: { bullish: bullishScore, bearish: bearishScore, total: 6 } };
};

// Расчет уровня уверенности
const calculateConfidenceLevel = (indicators, signal, conditionsMet) => {
    const { ema20, sma50, sma200, emaRsi, macd, bollingerBands, lastPrice, priceAction, hasPinBar } = indicators;
...
const calculateStopLossAndTakeProfit = (entryPrice, signal, volatility) => {
...
// Генерация сигналов с учетом стабильности
export const generateSignals = async (symbol, setCurrentPrices, setSignals, lastSignalChange, currentSignals) => {
    ...
    try {
        ...
        if (currentPrice) setCurrentPrices(prev => ({ ...prev, [symbol]: currentPrice }));
        ...
        // Проверяем время последнего изменения сигнала
        const now = new Date();
        const lastChange = lastSignalChange.current[symbol] || new Date(0);
        const timeSinceLastChange = now - lastChange;

        // Если сигнал изменился, но прошло мало времени - сохраняем предыдущий
        if (timeSinceLastChange < MIN_SIGNAL_DURATION && currentSignals[symbol]?.signalType !== signalType) {
            return currentSignals[symbol];
        }

        // Если сигнал действительно изменился - обновляем время
        if (signalType !== "HOLD" && (!currentSignals[symbol] || currentSignals[symbol].signalType !== signalType)) {
            lastSignalChange.current = { ...lastSignalChange.current, [symbol]: now };
        }
        ...
        if (signalType === "BUY") {
            ...
            if (indicators.emaRsi < 35) signal.reasons.push("Перепроданность (Сглаженный RSI < 35)");
            if (indicators.macd.histogram > 0.3 && indicators.macd.MACD > indicators.macd.signal)
                signal.reasons.push("Сильный бычий MACD");
            if (indicators.priceAction === "bullish") signal.reasons.push("Бычье ценовое действие");
            if (indicators.hasPinBar) signal.reasons.push("Пин-бар у уровня поддержки");
            ...
            if (indicators.emaRsi > 65) signal.reasons.push("Перекупленность (Сглаженный RSI > 65)");
            if (indicators.macd.histogram < -0.3 && indicators.macd.MACD < indicators.macd.signal)
                signal.reasons.push("Сильный медвежий MACD");
            if (indicators.priceAction === "bearish") signal.reasons.push("Медвежье ценовое действие");
            if (indicators.hasPinBar) signal.reasons.push("Пин-бар у уровня сопротивления");
        }

        setSignals(prev => ({ ...prev, [symbol]: signal }));

        if (signalType !== "HOLD" && analysis.conditionsMet.bullish + analysis.conditionsMet.bearish >= 2) {
            Swal.fire({
                icon: "info",
...
// Основной компонент
export const CryptoTreding = ({ symbols = ["BTC/USDT", "ETH/USDT", "BNB/USDT", "XRP/USDT", "SOL/USDT"] }) => {
    ...
    const lastSignalChange = React.useRef({});

    // Загрузка сохраненных сигналов при монтировании
    useEffect(() => {
        const loadSavedData = () => {
            try {
                const savedSignals = JSON.parse(localStorage.getItem('cryptoSignals')) || {};
                const savedLastChange = JSON.parse(localStorage.getItem('lastSignalChange')) || {};

                console.log('Loaded from localStorage:', savedSignals);

                setSignals(savedSignals);
                lastSignalChange.current = savedLastChange;
        ...
        const checkNewsTime = () => setIsNewsTime(isHighImpactNewsTime());
        checkNewsTime();
    ...
    // Сохранение сигналов в localStorage
    useEffect(() => {
        if (isInitialized && Object.keys(signals).length > 0) {
            console.log('Saving to localStorage:', signals);
            localStorage.setItem('cryptoSignals', JSON.stringify(signals));
            localStorage.setItem('lastSignalChange', JSON.stringify(lastSignalChange.current));
        }
    ...
    const analyzeAllSymbols = async () => {
        ...
        try {
            const promises = symbols.map(symbol => {
                ...
                return generateSignals(symbol, setCurrentPrices, setSignals, lastSignalChange, signals)
    ...
    const analyzeSymbol = async (symbol) => {
        ...
        setLoading(prev => ({ ...prev, [symbol]: true }));
        try {
            await generateSignals(symbol, setCurrentPrices, setSignals, lastSignalChange, signals);
            setLastUpdate(new Date());
        } catch (error) {
    ...
    const formatPrice = (symbol, price) => {
        ...
        if (symbol.includes("XRP") || symbol.includes("ADA")) return price.toFixed(4);
